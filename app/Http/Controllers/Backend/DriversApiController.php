...
class DriversApiController extends Controller {
	public function import_records(Request $request) {
		...
		if (count($errors) > 0) {
			...
			try {
				$file = $request->excel;
				$destinationPath = './assets/assets/samples/'; // upload path
				$extension = $file->getClientOriginalExtension();
				$fileName = Str::uuid() . '.' . $extension;
				$file->move($destinationPath, $fileName);
				Excel::import(new DriverImport, 'assets/assets/samples/' . $fileName);
				// $excel = Importer::make('Excel');
				// $excel->load('assets/assets/samples/' . $fileName);
	...
	public function profile() {
		...
		if (Auth::user()->getMeta('driver_image') != null) {
			...
			$src = asset("assets/assets/images/no-user.jpg");
		}
		$user = Auth::user();
