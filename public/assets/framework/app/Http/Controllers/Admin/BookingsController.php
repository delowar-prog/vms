			...
			return DataTables::eloquent($bookings)
				...
				->addColumn('return_booking', function ($row) {
					...
					if($row->getMeta('return_flag') == 1)
					{
						...
						if(isset($b))
						{
							return url('/assets/assets/customer_dashboard/assets/assets/img/return_way.svg');
						}
						else
						{
							return url('/assets/assets/customer_dashboard/assets/assets/img/one_way.svg');
						}
					}
					else
					{
					...
					if(isset($cus) && isset($driver))
					{
						...
						if($driver->fcm_id !=null)
						{
							...
							$notification =array(
								...
								// 'image' =>isset($custmerprofile) ? $custmerprofile : url('assets/assets/images/p2pride_mobile_app.png'),
	...
	public function push_notification($id) {
		...
		foreach ($select1 as $fetch) {
			...
			$array = array(
				'title' => $title ?? "",
				'body' => $body ?? "",
				'img' => url('assets/assets/images/' . Hyvikk::get('icon_img')),
				'url' => $url ?? url('admin/'),
			);
