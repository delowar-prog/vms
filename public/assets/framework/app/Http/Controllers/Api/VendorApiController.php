...
class VendorApiController extends Controller {
	...
	public function types(Request $request) {
		...
		foreach ($vehicle_types as $vehicle_type) {
			if ($vehicle_type->icon != null) {
				...
				$url = asset("assets/assets/images/vehicle.jpeg");
			}
			$vehicle_type_data[] = array(
				'id' => $vehicle_type->id,
	...
	private function yearly_income($year, $user_id) {
		...
		if ($user->group_id == null || $user->user_type == "S") {
			...
			$all_vehicles = VehicleModel::where('user_id', $user_id)->where('group_id', $user->group_id)->get();
			// $all_vehicles = VehicleModel::where('user_id', 3)->where('group_id',1)->get();
			// dd($all_vehicles);
	...
	public function years() {
		...
		$years = DB::table('income')
			->select(DB::raw('DISTINCT YEAR(date) as years'))
			->union(
				DB::table('expense')
					->select(DB::raw('DISTINCT YEAR(date) as years'))
			)
	...
	public function generate_pdf(Request $request) {
		...
		if (count($errors) > 0) {
			...
			$pdf->save('assets/assets/pdf/' . $filename);
			// $pdf->download($filename);
			$url = asset('assets/assets/pdf/' . $filename);
			$data['success'] = "1";
			$data['message'] = "data fetched";
	...
	public function edit_vehicle(Request $request) {
		...
		if (count($errors) > 0) {
			...
			$image = asset("assets/assets/images/vehicle.jpeg");
	...
	public function drivers(Request $request) {
		...
		foreach ($drivers as $row) {
			...
			$details[] = array(
				...
				'image' => ($row->driver_image) ? url('uploads/' . $row->driver_image) : url('assets/assets/images/user-noimage.png'),
		...
		if (count($errors) > 0) {
			...
			if (count($vendors) > 0) {
				foreach ($vendors as $vendor) {
					//dd($row);
					$src = ($vendor->photo != null) ? asset('uploads/' . $vendor->photo) : asset('assets/assets/images/no-user.jpg');
					$records[] = array(
						'vendor_id' => $vendor->id,
						'image' => $src,
						'name' => $vendor->name,
	...
	//         $data['data'] =  array(
	//             'success' => 1,
	//             'key' => 'upcoming_ride_notification',
	//             'message' => "New Ride has been Assigned to you.",
	//             'title' => "New Upcoming Ride for you !",
	//             // 'description' =>  $booking->pickup_addr . " - " . $booking->dest_addr . " on " . date('d-m-Y', strtotime($booking->pickup)) ,
	...
	//                     ->setDevicesToken('erRoaaTuTLe5Q0OnoDZCVI:APA91bHCt0fSvoYptftOxODd6W-xUfbLLlEOy03Yx8kDTxHSAv7zhK9UpA-bQW64cQ3FvgChkGRNlmqh39XGcBPJbeGscb1LgT7vffLbsJY7UmG3pHdcUrTjggMNcsGRZV87JWNI-mpm');
	//                     $push = $push->send();
	//                     dd($push);
